...
void DetourTransaction::add_detour(FuncType* source, const FuncType destination)
{
	...
	if (NO_ERROR != status)
	{
		throw MargAVitException(MargAVitStatus::MARGAVITSTATUS_DETOURTRANSACTION_ADD_DETOUR_DETOURATTACH_FAILED, static_cast<DWORD>(status));
	}
}
...
void DetourTransaction::remove_detour(FuncType* source, const FuncType destination)
{
	...
	if (NO_ERROR != status)
	{
		throw MargAVitException(MargAVitStatus::MARGAVITSTATUS_DETOURTRANSACTION_REMOVE_DETOUR_DETOURDETACH_FAILED, static_cast<DWORD>(status));
	}
}
