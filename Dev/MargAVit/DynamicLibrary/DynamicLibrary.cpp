...
PVOID DynamicLibrary::get_function(const std::string& function_name) const
{
	...
	{
		throw MargAVitException(MargAVitStatus::MARGAVITSTATUS_DYNAMICLIBRARY_GET_FUNCTION_GETPROCADDRESS_FAILED, GetLastError());
	}
...
HMODULE DynamicLibrary::_s_get_module(const std::string& module_name)
{
	...
	{
		throw MargAVitException(MargAVitStatus::MARGAVITSTATUS_DYNAMICLIBRARY_S_GET_MODULE_LOADLIBRARYA_FAILED, GetLastError());
	}
