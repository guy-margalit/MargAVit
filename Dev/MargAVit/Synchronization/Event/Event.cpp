...
void Event::set()
{
	...
	{
		throw MargAVitException(MARGAVITSTATUS_EVENT_SET_SETEVENT_FAILED, GetLastError());
	}
}
...
void Event::cancel()
{
	...
	{
		throw MargAVitException(MARGAVITSTATUS_EVENT_CANCEL_RESETEVENT_FAILED, GetLastError());
	}
}
...
HANDLE Event::_s_create_event(const std::optional<std::wstring>& event_name, const bool manual_reset, const bool initial_state)
{
	...
	{
		throw MargAVitException(MARGAVITSTATUS_EVENT_S_CREATE_EVENT_CREATEEVENTW_FAILED, GetLastError());
	}
